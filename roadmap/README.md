...
| 阶段 | 状态 | 目标 |
| --- | --- | --- |
| v0.1 Pi Runtime 与 EMI Control Plane 最小闭环 | 已完成 | 让一个小型 EMI 研发任务通过受控 Pi Runtime、持久化任务状态和外部工具边界，完成规格、执行和独立检查，到达人工验收点并保存证据 |
...
| 8 | 完成 v0.1 工程验收与发布 | 验收矩阵、已知限制、发布记录和下一轮真实场景校准入口 | 已完成 |

v0.1 第 1 至第 8 步工程建设已经完成。下一步选取一个真实、低风险且边界清楚的 EMI 研发任务，由用户和相应责任人确认 PRD、司法辖区、适用 Context 与验收标准后进行校准；Harness 不代替用户作出业务、法律或最终验收结论。

### 第 8 步实际结果

- 新增 [`CHANGELOG.md`](../CHANGELOG.md) 和 [`v0.1.0 工程验收与发布记录`](../docs/releases/v0.1.0.md)，逐项记录工程验收结论、证据、已知限制和下一阶段入口。
- 根工作区和 6 个实现包统一为 `0.1.0`；版本只表示工程基线，不表示生产就绪、法规解释确认或真实 EMI 业务验收通过。
- 从公开 GitHub 的发布候选提交 `dbf0119` 执行全新 clone、pnpm frozen-lockfile 安装和全工作区检查，确认仓库不依赖本机已有 `node_modules` 或未提交文件。
- 本地工作区与公开 clone 均通过结构、依赖边界、公共声明、类型检查、构建以及 20 个测试文件中的 55 个场景；外部目标检查和 TypeScript strict `--noEmit` 复核通过。
- 外部 `emi-pilot-ts` 目标仓库保持无远端且工作树干净；候选 Evidence Package 摘要复算一致，没有复制目标代码、数据库、Pi Session 或本地绝对路径到 Harness 仓库。
- 校准 Task 明确保留 `awaiting_acceptance` 和 `userDecisionRecorded: false`。仓库所有者确认的是 V0.1 工程版本命名与发布，不是对校准 Task 或未来真实 EMI 项目的业务验收。
- 工程版本使用 `v0.1.0` Git tag 识别；发布后从真实 EMI 场景开始校准，发现的问题带 Task、Run、Operation 和 Evidence 引用进入后续版本。
